  ...
  <section class="play-page">
    ...
    <template v-else-if="item">
      <div class="player-shell">
        <video v-if="isDirectMedia" ref="videoEl" class="media-player" :src="videoSrc" controls playsinline preload="metadata" @error="onPlayerError" />
      ...
      <div class="play-head"><div><h1>{{ item.title }}</h1><p>{{ item.typeName || '视频内容' }} <span v-if="item.year">· {{ item.year }}</span></p></div><button class="secondary-btn" @click="$router.push(`/detail/${item.id}`)">详情</button></div>
...
const route=useRoute(); const item=ref(null); const loading=ref(true); const activeSourceIndex=ref(0); const activeEpisodeIndex=ref(0); const playerError=ref(''); const videoEl=ref(null)
const sources=computed(()=>parsePlaySources(item.value)); const activeSource=computed(()=>sources.value[activeSourceIndex.value]||null); const activeEpisode=computed(()=>activeSource.value?.episodes[activeEpisodeIndex.value]||null); const isDirectMedia=computed(()=>/\.(mp4|webm|ogg|m3u8)(?:\?|$)/i.test(activeEpisode.value?.url||'')); const videoSrc=computed(()=>activeEpisode.value?.url||'')
...
function onPlayerError(){playerError.value='当前地址无法直接播放，请切换线路或使用外部播放器'}
watch(activeEpisode,async()=>{saveCurrent();await nextTick();if(videoEl.value){videoEl.value.load();videoEl.value.play().catch(()=>{})}})
