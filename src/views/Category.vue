<template>
  <section class="page-view">
    ...
    <div v-else-if="videos.length" class="video-grid"><article v-for="item in videos" :key="item.id" class="video-card" @click="open(item.id)"><div class="thumb"><img :src="item.poster || fallback" :alt="item.title" loading="lazy" @error="fallbackImage"><span v-if="item.remarks" class="badge">{{ item.remarks }}</span><span class="duration">{{ item.duration || 'HD' }}</span><button class="play" aria-label="打开">▶</button></div><h3>{{ item.title }}</h3><div class="stats"><span>◉ {{ formatViews(item.views) }}</span><span class="score">★ {{ item.score || '—' }}</span></div></article></div>
...
function formatViews(v){const n=Number(v)||0;return n>999999?`${(n/1000000).toFixed(1)}M`:n>999?`${(n/1000).toFixed(1)}K`:String(n)}function fallbackImage(e){e.target.src=fallback}function open(id){if(id)router.push(`/detail/${id}`)}
function syncPage(){const n=Number(route.query.pg);page.value=Number.isFinite(n)&&n>0?Math.min(Math.trunc(n),100000):1}
async function load(){const token=++requestToken;loading.value=true;try{if(!classes.value.length)classes.value=await getClasses().catch(()=>[]);if(token!==requestToken)return;const result=props.latest?await getLatestVideos(page.value,18):props.popular?await getHotVideos(page.value,18):await getCategoryVideos(route.params.id,page.value,18);if(token!==requestToken)return;videos.value=result.list;pageCount.value=Math.max(1,result.pageCount||1);if(page.value>pageCount.value){page.value=pageCount.value;router.replace({query:{...route.query,pg:page.value}})}}catch{if(token===requestToken)videos.value=[]}finally{if(token===requestToken)loading.value=false}}
