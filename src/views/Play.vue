<template>
  <main class="play-page">
    <div class="play-layout">
      <section class="player-column">
        <div class="player-card">
          <div class="player-topbar"><div class="player-status"><span></span>正在播放</div><div class="player-format">HD · HLS · 10秒快进</div></div>
          <div class="player-wrap"><div v-if="loading" class="player-loading">正在加载视频…</div><video v-else ref="videoEl" class="player" playsinline controls preload="metadata"></video></div>
        </div>
        <section v-if="item" class="video-info-card"><div class="info-header"><div class="title-area"><div class="eyebrow"><span></span>NOW PLAYING</div><h1 class="video-title">{{item.title}}</h1></div><button type="button" class="share-btn" @click="sharePage">分享</button></div><div class="video-meta"><span>高清在线播放</span><span v-if="item.views">{{formatViews(item.views)}} 次播放</span><span v-if="item.typeName">{{item.typeName}}</span><span v-if="item.updateTime">{{item.updateTime}}</span></div><div v-if="item.content" class="description"><div class="description-title">视频简介</div><div class="description-text">{{item.content}}</div></div></section>
      </section>
      <aside class="side-panel"><div class="side-card"><div class="side-heading"><div><span class="side-kicker">EXPLORE</span><h2>相关推荐</h2></div><span class="recommend-count">{{recommendations.length}}</span></div><div v-if="recommendations.length" class="side-list"><router-link v-for="video in recommendations.slice(0,6)" :key="video.id" :to="`/play/${video.id}`" class="side-item"><div class="side-thumb"><img :src="video.poster || fallback" :alt="video.title" loading="lazy"><span class="side-play">▶</span></div><div class="side-item-info"><strong>{{video.title}}</strong><span>{{formatViews(video.views)}} 次播放</span></div></router-link></div><div v-else class="side-empty">暂无相关推荐</div></div></aside>
    </div>
    ...
    <section v-if="recommendations.length" class="recommend-section"><div class="section-bar"><div><div class="section-kicker">MORE FOR YOU</div><h2 class="section-heading">猜你喜欢</h2><div class="section-subtitle">更多同类精彩内容</div></div><span class="section-count">{{Math.min(recommendations.length,20)}} 部</span></div><div class="video-grid"><VideoCard v-for="video in recommendations.slice(0,20)" :key="video.id" :item="video" /></div></section>
    <div v-if="!sources.length && item" class="empty play-empty">该内容没有可识别的播放地址</div>
    <div v-if="!item && !loading" class="empty">播放器内容暂不可用</div>
  </main>
</template>
<script setup>
...
const route=useRoute();const router=useRouter();const item=ref(null);const loading=ref(true);const playerError=ref('');const videoEl=ref(null);const player=ref(null);const hls=ref(null);const recommendations=ref([]);const activeSourceIndex=ref(0);const activeEpisodeIndex=ref(0);const fallback='/fallback.jpg'
