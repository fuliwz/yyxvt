...
export function saveHistory(item, episode = null) {
  ...
  list.unshift({
    ...
    poster: String(item.poster || '').slice(0, 2048),
    typeName: String(item.typeName || '').slice(0, 100),
    episode: String(episode?.label || '').slice(0, 100),
    url: String(episode?.url || '').slice(0, 4096),
    updatedAt: Date.now()
  })
  write(list)
}
