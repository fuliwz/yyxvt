  ...
  <section class="home">
    <section class="section popular-section">
      ...
      <div ref="popularRow" class="video-row">
        <article v-for="item in popular" :key="item.id" class="mini-card" @click="open(item.id)"><div class="thumb"><img :src="item.poster || fallback" :alt="item.title" loading="lazy" @error="fallbackImage"><span v-if="item.remarks" class="badge">{{ item.remarks }}</span><span class="duration">{{ item.duration || 'HD' }}</span><div class="thumb-meta"><span>◉ {{ formatViews(item.views) }}</span><span>{{ item.year || '' }}</span></div></div><div class="update-time">{{ formatUpdateTime(item) }}</div><h3 class="mini-title">{{ item.title }}</h3></article>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div v-else-if="videos.length" class="video-grid">
        <article v-for="item in videos" :key="item.id" class="video-card" @click="open(item.id)"><div class="thumb"><img :src="item.poster || fallback" :alt="item.title" loading="lazy" @error="fallbackImage"><span v-if="item.remarks" class="badge">{{item.remarks}}</span><span class="duration">{{item.duration || 'HD'}}</span><button class="play" aria-label="打开">▶</button></div><div class="update-time">{{ formatUpdateTime(item) }}</div><h3>{{item.title}}</h3><div class="stats"><span>◉ {{formatViews(item.views)}}</span><span class="score">★ {{item.score || '—'}}</span></div></article>
      </div>
...
function formatUpdateTime(item){const raw=item?.raw||{};const value=raw.vod_time||raw.vod_time_add||raw.vod_time_hits||raw.vod_pubdate||raw.vod_up||raw.vod_addtime||'';if(!value)return item?.year?'更新于 '+item.year:'最新更新';const date=new Date(String(value).replace(/-/g,'/'));if(Number.isNaN(date.getTime()))return String(value);const diff=Math.max(0,Date.now()-date.getTime());const minute=Math.floor(diff/60000),hour=Math.floor(minute/60),day=Math.floor(hour/24);if(minute<1)return '刚刚更新';if(minute<60)return `${minute}分钟前更新`;if(hour<24)return `${hour}小时前更新`;if(day<30)return `${day}天前更新`;return `更新于 ${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`}
async function load(){loading.value=true;try{const [main,hot]=await Promise.all([activeTab.value==='latest'?getLatestVideos(1,18):getHotVideos(1,18),getHotVideos(1,8)]);videos.value=main.list;popular.value=hot.list}catch{videos.value=[];popular.value=[]}finally{loading.value=false}}
