...
function go(n){const next=Math.max(1,Math.min(Number(n)||1,pageCount.value));if(next===page.value)return;router.push({query:{...route.query,pg:next}})}function open(id){if(id)router.push(`/play/${id}`)}
