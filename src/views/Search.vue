...
async function load(){const token=++requestToken;if(!keyword.value){videos.value=[];pageCount.value=1;loading.value=false;return}loading.value=true;try{const r=await searchVideos(keyword.value,page.value,18);if(token!==requestToken)return;videos.value=r.list;pageCount.value=Math.max(1,r.pageCount||1);if(page.value>pageCount.value){page.value=pageCount.value;router.replace({query:{...route.query,pg:page.value}})}}catch{if(token===requestToken)videos.value=[]}finally{if(token===requestToken)loading.value=false}}
